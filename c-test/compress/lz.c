...
uint32_t lz77_compress (uint8_t *uncompressed_text, uint32_t uncompressed_size, uint8_t *compressed_text)
{
    ...
    uint32_t compressed_pointer, output_size=1, coding_pos, output_lookahead_ref, look_behind, look_ahead;
...
uint32_t file_lz77_compress (char *filename_in, char *filename_out)
{
    ...
    uint32_t uncompressed_size, compressed_size,t;
    
    in = fopen(filename_in, "r");
    if(in == NULL)
        return 0;
    uncompressed_size = fsize(in);
    printf("__uncompressed_size=%d\n", uncompressed_size);
    uncompressed_text = malloc(uncompressed_size);
    t=fread(uncompressed_text, 1, uncompressed_size, in);
    printf("t=%d\n", t);
    if((uncompressed_size != t))
        return 0;
    fclose(in);
    ...
    printf("__compressed_size=%d\n", compressed_size);
    
    out = fopen(filename_out, "w");
    if(out == NULL)
        return 0;
    t=fwrite(compressed_text, 1, compressed_size, out);
    if((compressed_size != t)){
        printf("t=%d\n", t);
        return 0; 
    }
...
int main(){
    ...
    FILE *in;
    in = fopen("passport.bmp", "r");
    if(in == NULL)
        return 0;
    printf("Original size: %ld\n", fsize(in));
    fclose(in);
    printf("Compressed: %u\n", file_lz77_compress("passport.bmp", "passport.bmp.z77"));
    printf("Decompressed: %u", file_lz77_decompress("passport.bmp.z77", "passport-2.bmp"));
}